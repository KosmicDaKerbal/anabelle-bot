...
module.exports = {
  ...
  async execute (interaction, objectTypeCode) {
    ...
    captchaCommand.on("success", async data => {
      ...
      captchaEmbed.setTitle(`ẁ̸̡̛̱e̸̬̙̚l̴̏̇ͅĉ̴͎͘ọ̷̄̒m̵̻̬̅͝ȅ̶̹͝`).setDescription(`<@${data.member.user.id}> i̶͝ͅs̴̹̚ ̸̘́h̶͚͗e̵̛̼r̸͈͛ë̷̫́ ̴͎̿t̷̙̓o̸̜̐ ̷̺̀p̵̜͗l̴̮̓a̸̬͗y̸̬̆`);
        ...
        console.log(`[INFO] CAPTCHA for ${data.member.user.username} answered incorrectly, ID: ${data.member.user.id}`);
            await guild.members.cache.get(data.member.user.id).then((member) => {
              if (!member){
                ...
                captchaEmbed.setTitle("Captcha Fail").setDescription(`To retry, Type /captcha in the <#${process.env.CAPTCHA_CHANNEL_ID}> channel.`).setFooter({ text: 'Captcha verification failed'}).setTimestamp();
        ...
        console.log(`[INFO] CAPTCHA for ${data.member.user.username} timed out, ID: ${data.member.user.id}`);
            await guild.members.cache.get(data.member.user.id).then((member) => {
              if (!member){
                ...
                captchaEmbed.setTitle("Captcha Timeout").setDescription(`To retry, Type /captcha in the <#${process.env.CAPTCHA_CHANNEL_ID}> channel.`).setFooter({ text: 'Captcha verification timed out'}).setTimestamp();
                interaction.client.users.send(data.member.user.id, { embeds: [captchaEmbed] }).catch((e)=>{
                console.log(`[INFO] ${data.member.user.username} does not allow DM's from bots, ID: ${data.member.user.id}`);
                });
              }
