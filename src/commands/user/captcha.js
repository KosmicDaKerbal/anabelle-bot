...
module.exports = {
  ...
  async execute (interaction, objectTypeCode) {
    ...
    const localData = interaction.client.db.prepare("SELECT unverifiedRoleID, verifiedRoleID, verificationChannelID, welcomeChannelID, logChannelID FROM localConfig WHERE guildID = ?").get(guild.id);
    ...
    if (!objectTypeCode && interaction.member.roles.cache.has(localData.verifiedRoleID)) captchaEmbed.setTitle("User already verified").setDescription("You have texting permissions on the server.").setColor(0x00ff00);
    else {
      try {
    ...
    captchaCommand.on("success", async data => {
      const vchannel = await interaction.client.channels.fetch(localData.welcomeChannelID);
      ...
      data.member.roles.remove(localData.unverifiedRoleID);
    });
        ...
        console.log(`[INFO] CAPTCHA for ${data.member.user.username} answered incorrectly, ID: ${data.member.user.id}`);
            await guild.members.fetch(data.member.user.id).then((member) => {
              if (!member){
                ...
                captchaEmbed.setTitle("Captcha Fail").setDescription(`To retry, Type /captcha in the <#${localData.verificationChannelID}> channel.`).setFooter({ text: data.member.guild.name, iconURL: data.member.guild.iconURL({ dynamic: true, size: 32 })}).setTimestamp().setColor(0xff0000);
        ...
        console.log(`[INFO] CAPTCHA for ${data.member.user.username} timed out, ID: ${data.member.user.id}`);
            await guild.members.fetch(data.member.user.id).then((member) => {
              if (!member){
                ...
                captchaEmbed.setTitle("Captcha Timeout").setDescription(`To retry, Type /captcha in the <#${localData.verificationChannelID}> channel.`).setFooter({ text: data.member.guild.name, iconURL: data.member.guild.iconURL({ dynamic: true, size: 32 })}).setTimestamp().setColor(0xff0000);
                interaction.client.users.send(data.member.user.id, { embeds: [captchaEmbed] }).catch((e)=>{
                console.log(`[INFO] ${data.member.user.username} does not allow DM's from bots, ID: ${data.member.user.id}`);
                });
              }
