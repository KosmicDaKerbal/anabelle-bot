...
client.on(Events.GuildMemberAdd, async member => {
  ...
  const membersWithPermission = (await member.guild.members.fetch()).filter((m => m.permissions.has(PermissionsBitField.Flags.ManageGuild)));
...
client.on(Events.InteractionCreate, async (mainInteraction) => {
  ...
  for (const key in localData) if (localData[key] === null) nullKeys.push(key);
    ...
    await mainInteraction.reply({ embeds: [index], flags: MessageFlags.Ephemeral });
		return;
	} else if ((nullKeys.length > 0 || !localData) && mainInteraction.commandName != 'config'){
